...
public class StencilMask {
	...
	public static void initStencil(){
        ...
        glPushMatrix();
        glLoadIdentity();
        glOrtho(0, MainFrameRefactored.settings.resWidth, 0, MainFrameRefactored.settings.resHeight, 0.2f, 3);
        ...
        glColor4f(1,1,1,1);
        for(int i=0;i<MainFrameRefactored.settings.resHeight;i+=2){
            glLineWidth(1);
            glBegin(GL_LINES);
            glVertex2f(0,i);
            glVertex2f(MainFrameRefactored.settings.resWidth,i);
            glEnd();
        }
        //exit ortho drawing
        glMatrixMode(GL_PROJECTION);
