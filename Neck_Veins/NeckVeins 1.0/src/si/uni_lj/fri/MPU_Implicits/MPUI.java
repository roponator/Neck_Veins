...
public class MPUI
{
	...
	public MPUI(float alpha, float lambda, float error, boolean cubes, float res, float[] points, float[] normals, int xDim, int yDim, int zDim)
	{
		...
		NiftyScreenController.UpdateLoadingBarDialog("Loading model...", 40.0f);
		VeinsWindow.veinsWindow.RenderSingleFrameWithoutModel();

		startTime = System.nanoTime();
		// Serial bloomenthal
		if (cubes)
			initializePoligonizationCube(res, xDim, yDim, zDim);
		else
			initializePoligonization(res, xDim, yDim, zDim);

		NiftyScreenController.UpdateLoadingBarDialog("Loading model...", 45.0f);
		VeinsWindow.veinsWindow.RenderSingleFrameWithoutModel();

		/*
		 * globalImplicit.primitiveTreeConstruction(); System.out.println(globalImplicit.getFnBuff().length); try { Object[] out = BloomenthalGPU.createModel(0.0025f, globalImplicit); IntBuffer nTrianglesBuff = (IntBuffer) out[0]; FloatBuffer trianglesBuff = (FloatBuffer) out[1]; FloatBuffer
		 * normalsBuf = (FloatBuffer) out[2]; outputVertices = new float[nTrianglesBuff.get(0)*9]; // outputNormals = new float[nTrianglesBuff.get(0)*9]; for(int i = 0; i<nTrianglesBuff.get(0)*9; i++){ outputVertices[i] = trianglesBuff.get(i); //outputNormals[i] = null; } } catch (LWJGLException e)
