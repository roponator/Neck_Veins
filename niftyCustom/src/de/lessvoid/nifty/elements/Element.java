...
public class Element implements NiftyEvent, EffectManager.Notify
{
	...
	/**
	 * This is the shared instance of the comparator used to get the rendering oder for the child elements of another element.
	 */
	@Nonnull
	...
	/**
	 * Effect state cache (this includes info about the child state) and is only update when Effect states are changed.
	 */
	@Nonnull
	...
	/**
	 * This helps us to keep track when you enable or disable this multiple times. We don't want to start the onEnabled/onDisabled effects when the element is already enabled/disabled.
	 */
	...
	/**
	 * this is set to true, when there's no interaction with the element possible. this happens when the onEndScreen effect starts.
	 */
	private boolean done;

	/**
	 * this is set to true when there's no interaction with this element possibe. (as long as onStartScreen and onEndScreen events are active even when this element is not using the onStartScreen effect at all but a parent element did)
	 */
	...
	/**
	 * This attribute determines the element before (!) the new element will be inserted into the focusHandler. You can set this to any element id on the screen and this element will be inserted right before the given element. This is especially useful when you dynamically remove and add elements
	 * and you need to enforce a specific focus order.
	 * <p/>
	...
	public Element(@Nonnull final Nifty nifty, @Nonnull final ElementType elementType, @Nullable final String id, @Nullable final Element parent, @Nonnull final FocusHandler focusHandler, final boolean visibleToMouseEvents, @Nonnull final TimeProvider timeProvider,
	 ...
	 */
	public Element(@Nonnull final Nifty nifty, @Nonnull final ElementType elementType, @Nullable final String id, @Nullable final Element parent, @Nonnull final LayoutPart layoutPart, @Nonnull final FocusHandler focusHandler, final boolean visibleToMouseEvents,
			@Nonnull final TimeProvider timeProvider, @Nonnull final ElementRenderer... elementRenderer)
	{
		this.nifty = nifty;
	 ...
	 */
	public void initializeFromAttributes(@Nonnull final Screen targetScreen, @Nonnull final Attributes attributes, @Nonnull final NiftyRenderEngine renderEngine)
	{
	...
	public String getElementStateString(@Nonnull final String offset, @Nonnull final String regex)
	{
		...
		elementDebugOut.add(" constraint [" + outputSizeValue(layoutPart.getBoxConstraints().getX()) + ", " + "" + outputSizeValue(layoutPart.getBoxConstraints().getY()) + ", " + outputSizeValue(layoutPart.getBoxConstraints().getWidth()) + ", "
				+ outputSizeValue(layoutPart.getBoxConstraints().getHeight()) + "]");
		elementDebugOut.add(" padding [" + outputSizeValue(layoutPart.getBoxConstraints().getPaddingLeft()) + ", " + "" + outputSizeValue(layoutPart.getBoxConstraints().getPaddingRight()) + ", " + "" + outputSizeValue(layoutPart.getBoxConstraints().getPaddingTop()) + ", " + ""
				+ outputSizeValue(layoutPart.getBoxConstraints().getPaddingBottom()) + "]");
		elementDebugOut.add(" margin [" + outputSizeValue(layoutPart.getBoxConstraints().getMarginLeft()) + ", " + "" + outputSizeValue(layoutPart.getBoxConstraints().getMarginRight()) + ", " + "" + outputSizeValue(layoutPart.getBoxConstraints().getMarginTop()) + ", " + ""
		...
		if (children != null && elementsRenderOrderSet != null)
		{
			...
			{
				renderOrder.append("[").append(e.id).append(" (").append((e.renderOrder == 0) ? children.indexOf(e) : e.renderOrder).append(")]");
			}
		}
	...
	public void registerEffect(@Nonnull final EffectEventId theId, @Nonnull final Effect e)
	{
	...
	public void startEffect(@Nonnull final EffectEventId effectEventId, @Nullable final EndNotify effectEndNotify, @Nullable final String customKey)
	{
	...
	public void startEffectWithoutChildren(@Nonnull final EffectEventId effectEventId, @Nullable final EndNotify effectEndNotify)
	{
		startEffectWithoutChildren(effectEventId, effectEndNotify, null);
	}

	public void startEffectWithoutChildren(@Nonnull final EffectEventId effectEventId, @Nullable final EndNotify effectEndNotify, @Nullable final String customKey)
	{
		startEffectDoIt(effectEventId, effectEndNotify, customKey, false);
	}

	private void startEffectDoIt(@Nonnull final EffectEventId effectEventId, @Nullable final EndNotify effectEndNotify, @Nullable final String customKey, final boolean withChildren)
	{
		...
		// as well.
		if (effectEventId != EffectEventId.onShow || (effectEventId == EffectEventId.onShow && visible))
		{
	...
	private void startEffectInternal(@Nonnull final EffectEventId effectEventId, @Nullable final EndNotify effectEndNotify, @Nullable final String customKey)
	{
		if (effectEventId == EffectEventId.onStartScreen)
		{
			if (!visible)
	...
	/**
	 * Returns true if this element is visible. Please note that this is with regards to that element only. It's possible that this element is invisible (because of any of its parent elements is invisible) and still this method will return true;
	 *
	...
	/**
	 * This is a special version of canHandleMouseEvents() that will return true if this element is able to process mouse events in general. This method will return true even when the element is temporarily not able to handle events because onStartScreen or onEndScreen effects are active.
	 *
	 ...
	 */
	public void buildMouseOverElements(@Nonnull final NiftyMouseInputEvent mouseEvent, final long eventTime, @Nonnull final MouseOverHandler mouseOverHandler)
	{
	...
	{
		return mouseX >= parentClipX && mouseX <= (parentClipX + parentClipWidth) && mouseY > (parentClipY) && mouseY < (parentClipY + parentClipHeight);
	}
	 ...
	 * Simulates a click-release of the primary mouse button on the element.
	 *
	 * This method is called automatically in many cases as a response to receiving a {@link de.lessvoid.nifty.input.NiftyStandardInputEvent#Activate} event.
	 *
	 ...
	 *
	 * 2) When a {@link de.lessvoid.nifty.screen.Screen} is starting or ending, or more specifically, when: {@link de.lessvoid.nifty.effects.EffectEventId#onStartScreen} or {@link de.lessvoid.nifty.effects.EffectEventId#onEndScreen} effects are active on the current
	 * {@link de.lessvoid.nifty.screen.Screen}.
	 *
	 * 3) If there is no current {@link de.lessvoid.nifty.screen.Screen}, i.e., the current {@link de.lessvoid.nifty.screen.Screen} is null.
	 *
	 ...
	 *
	 * @param <T>
	 *            the {@link de.lessvoid.nifty.elements.render.ElementRenderer} type to check for
	 * @param requestedRendererClass
	 *            the {@link de.lessvoid.nifty.elements.render.ElementRenderer} class to check for
	 * @return the {@link de.lessvoid.nifty.elements.render.ElementRenderer} that matches the specified class, or null if there is no matching renderer
	 */
	@Nullable
	...
	private boolean hasParentActiveOnStartOrOnEndScreenEffect()
	{
		if (parent != null)
		{
			return parent.effectManager.isActive(EffectEventId.onStartScreen) || parent.effectManager.isActive(EffectEventId.onEndScreen) || parent.hasParentActiveOnStartOrOnEndScreenEffect();
		}
		return false;
	}
	...
	public class LocalEndNotify implements EndNotify
	{
		...
		public LocalEndNotify(@Nonnull final EffectEventId effectEventIdParam, @Nullable final EndNotify effectEndNotifyParam)
		{
		...
		public void perform()
		{
			...
			{
				if (interactionBlocked && !hasParentActiveOnStartOrOnEndScreenEffect() && !isEffectActive(effectEventId))
				{
	...
	public void setId(@Nullable final String id)
	{
		...
		/*
		 * So the ID changed and we got a parent. This means the render order set is likely to be corrupted now. We need to update it to ensure that everything is still working properly.
		 */
		parent.renderOrderChanged(this);
	}
	...
	@Nullable
	public <T extends Controller> T findControl(@Nonnull final String elementName, @Nonnull final Class<T> requestedControlClass)
	{
	...
	@Nullable
	public <T extends NiftyControl> T findNiftyControl(@Nonnull final String elementName, @Nonnull final Class<T> requestedControlClass)
	{
		Element element = findElementById(elementName);
		if (element == null)
		{
	...
	public <T extends NiftyControl> T getNiftyControl(@Nonnull final Class<T> requestedControlClass)
	{
		...
		log.warning("missing element/control with id [" + id + "] for requested control class [" + requestedControlClass.getName() + "]");
		return null;
	}
	...
	@Nonnull
	public <T extends EffectImpl> List<Effect> getEffects(@Nonnull final EffectEventId effectEventId, @Nonnull final Class<T> requestedClass)
	{
	...
	/**
	 * This uses the renderOrder attribute of the elements to compare them. If the renderOrder attribute is not set (is 0) then the index of the element in the elements list is used as the renderOrder value. This is done to keep the original sort order of the elements for rendering. The value is not
	 * cached and is directly recalculated using the element index in the list. The child count for an element is usually low (< 10) and the comparator is only executed when the child elements change. So this lookup shouldn't hurt performance too much.
	 * <p/>
	 * If you change the default value of renderOrder then your value is being used. So if you set it to some high value (> 1000 to be save) this element is rendered after all the other elements. If you set it to some very low value (< -1000 to be save) then this element is rendered before all the
	 * others.
	 */
